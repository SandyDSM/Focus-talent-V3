...
function Pagination({MaxPpagina, BackPag, NextPag, PActual, MaxPaginas, SeteoActual}) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-2 justify-end">
      <div className="flex justify-center">
        <SelectField width={"100%"} onChange={/* aqui agregué la funcion para cuando cambia de numero de registros*/(e)=>MaxPpagina(e.target.value)}>
      ...
      <div className="flex">
        <Button variation="link" size="small" onClick={/*las funciones clic para cada boton */ ()=>{SeteoActual(0); botones(1,false);}}>
          <Page2Arrow></Page2Arrow>
        </Button>
        <Button variation="link" size="small" onClick={()=>{
          if(PActual>0){
          BackPag();
...
{/*Aqui agregue id para los 3 botones y sus funciones clic */}
        <Button style={estilo} size="small" id="primero" onClick={(e)=>moveTo(e.target.innerHTML, e.target.id)}>1</Button>
        <Button size="small"  id="segundo" onClick={(e)=>moveTo(e.target.innerHTML, e.target.id)}>2</Button>
        <Button size="small"  id="tercero" onClick={(e)=>moveTo(e.target.innerHTML, e.target.id)}>3</Button>
        ...
        <Button variation="link" size="small" onClick={()=>{SeteoActual(MaxPaginas-1); botones(MaxPaginas,true); }}>
